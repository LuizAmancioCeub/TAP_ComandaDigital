...
@Component({
  selector: 'app-model-adicionar-item',
  templateUrl: './model-adicionar-item.component.html',
  styleUrls: ['./model-adicionar-item.component.css']
})
export class ModelAdicionarItemComponent implements OnInit {
  ...
  onSubmitItem(){
    ...
    if (this.nomeItem.trim() !== '' && this.descricao.trim() !== '' &&!isNaN(this.preco) && !isNaN(this.categoria)){
      this.axiosService.request(
        ...
        this.close();
        setTimeout(() => {
          this.eventService.emitMsg(true, "Item adicionado com Sucesso");
        }, 300);
      }).catch((error) => {
